...
(define rule-type
  (lambda (rtype)
	...
	[draft-at-least-2 (not (draft-at-least-two-test))]
...
(define draft-at-least-two-test
  (lambda ()
    (let
	([old-q-ls *quanta-list*])
      (set! *quanta-list* (append (if (null? *own-stuff*)
				      '()
				      (cdr (reverse *own-stuff*)))
				  *other-stuff*))
      (let
	  ([answer1 (at-least-two-test)])
	(set! *quanta-list* (append *other-stuff* *own-stuff*))
	(let
	    ([answer2 (at-least-two-test)])
	  (set! *quanta-list* old-q-ls)
	  (and answer1 answer2))))))
