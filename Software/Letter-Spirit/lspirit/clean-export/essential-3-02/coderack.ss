...
; number of codelets per loosening phase in Examiner
; twice as long didn't help much
(set! *exam-phase* 800)

(set! *max-codelets* (* 10 *exam-phase*)) ; Adjudicator never runs this long
...
;	(printf "Going to phase 2~%")
	(if (> (full-levels) 1)
	    (begin
	      (post-bridge-aheads 150) ; was up to 500 of each
	      (post-bridge-backs 150))
	    (begin
	      (post-bridge-aheads 100) ; was up to 500 of each
	      (post-bridge-backs 100)))   
	(set! *phase* 2)
	(adj-run))]
...
;	(printf "Going to phase 3~%")
	...
	(set! *phase* 3)
	(adj-run))]
     [(and (eq? *phase* 3) (or (null? *coderack*) (null? *bridges*)))
      (begin
;	(printf "Phase 3, no codelets, no bridges~%")
	(map filler-reward *fillers*)
;	(printf "Temperature ~s~%" (round *temperature*))
	(set! *promotions* (reverse *promotions*)) ; FIFO, not LIFO
...
(define quit-score
  (lambda (scores)
    (round-3
     (+ (nth 1 scores) (nth 2 scores) (nth 3 scores)
	(nth 4 scores) (nth 6 scores) (nth 7 scores)
	(nth 9 scores)))))

(define continue-score
  (lambda (scores)
    (round-3
     (+ (nth 1 scores) (* -0.025 (nth 2 scores)) (nth 3 scores)
	(nth 5 scores) (nth 6 scores) (nth 7 scores)
	(nth 8 scores)))))

(define choice-info
  (lambda ()
    (map list
	 (map car *choice-weights*)
	 (map continue-score (map cdr *choice-weights*))
	 (map quit-score (map cdr *choice-weights*)))))
...
(define draft-winner
  (lambda ()
    (let*
	([raw-index (choice-info)]
	 [steps (map cadr *candidates*)]
	 [score-stuff
	  (lambda (step)
	    (lookup-list step raw-index))]
	 [score-stuffs (map score-stuff steps)]
	 [index (map cons *candidates* score-stuffs)]
	 [len (length index)]
	 [scores (map list
		      (n-copies 'max len)
		      (map cadr index)
	 ...
	 [diminish
	  (lambda (n)
	    (* n 0.000001))]
	 [best-scores-index (map list
				 (map car index)
				 (map diminish (map eval scores)))]
	 [champ (find-max best-scores-index)]
	 [bottomline-index
	  (score-lists-add
	   (list (list champ 25)) ; play with this number - 2 is not small!
	   best-scores-index)])
      (weight-to-n-roulette bottomline-index *choose-rand*))))

(set! *choose-rand* 25) ; was 25 -- low is more random

; to evaluate, for each choice, what the better of its two outcomes is
; scorelist has items of the format (item score1 score2)
(define best-choice
  (lambda (quantum scorelist)
    (let
	([scores (lookup-list quantum scorelist)])
      (apply max scores))))

(define prune-candidate
  (lambda (key score best)
    (let
	([post-check (if (> (- best score) 4000)
			 (remove-key key *candidates*)
			 *candidates*)])
	  (set! *candidates* post-check))))

(define prune-candidates
  (lambda ()
    (if (not (null? *candidates*))
	(let*
	    ([score-ls *candidates*]
	     [quanta (map cadr score-ls)]
	     [choice-scores (choice-info)]
	     [best-alive
	      (lambda (item)
		(best-choice item choice-scores))]
	     [best-scores (map best-alive quanta)]
	     [pt-scores (map list (map car score-ls) best-scores)]
	     [very-best (apply max best-scores)]
	     [prune-one
	      (lambda (item)
		(prune-candidate (car item) (cadr item) very-best))])
	  (map prune-one pt-scores)))))
    ...
    (cond
     [(null? *coderack*)
	  ...
	  ([winner
		...
		0
		(draft-winner))])
	(set! *last-point* *current-point*)
	(if (or
	     (eq? winner 0)
	     (> (+ 250 *quit-consider*) ; favors early quit - OK???
		     (apply max (lookup-list (cadr winner) (choice-info)))))
	    (set! *quit-drawing* #t)
	    (begin
	      (set! *quit-consider*
		    (cadr (lookup-list (cadr winner) (choice-info))))
	      (set! *current-point* (car winner))
	    ;  (printf "  ~s" (cadr winner))
	      (set! *own-stuff*
		    (append *own-stuff* (list (cadr winner))))
	      (if *busy-draft*
		  (quantum-place (get-category *the-whole*) (cadr winner)))
	      (set! *quanta-list*
		    (append *quanta-list* (list (cadr winner)))))))]
     [else (rackprint *workspace*)
	   (if *graphics* (begin (draw-workspace *workspace*)
				 (if *display-coderack*
				     (draw-coderack *coderack*))
	   ...
	   (if
	       (and
		*busy-draft*
		(eq? 0 (mod *codelets-run* *graf-step*)))
	       (let*
		   ([pos (positive-candidates)]
		    [show-it (if (null? pos) '() (roulette pos))]
		    [cat (get-category *the-whole*)])
		 (if (not (null? pos))
		     (begin
		       (show-quantum cat show-it)
		       (unshow-quantum cat show-it)))))
	   (draft-run)])))
