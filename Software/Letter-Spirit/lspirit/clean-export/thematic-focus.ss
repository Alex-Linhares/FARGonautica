...
(define library-entry
  (lambda ()
     ...
     *answer*
     *fillers*
     (map list *sp-types* (map eval *sp-types*)))))
...
(define importance
  (lambda (sp-type level sp)
	...
	([per-sp
	  (case sp-type
	    [*abstract-rules* 5.0] ; was 4.0
	    [*val-to-val-nvs* 5.0] ; was 4.0
	    [*relative-nvs* 0.6] ; was 0.5
...
(define sp-prob
  (lambda (sp-type sp)
    (case sp-type
      ...
      [*val-to-val-nvs* 0.08]
      [*relative-nvs* 0.25]
      [*literal-motifs* (power 0.35 (length sp))] ; was 0.18
...
; LATE CHANGE! took out -1 from the last line
(define draft-level-score
  (lambda (sp-type level sp)
    (max 0 (- (level-score level) (level-expect sp-type sp)))))
...
(define library-fillers
  (lambda ()
	...
	'()
	(map list
	     (map car (car (map cadddr *library*)))
	     (map canonicalize-q-ls
		  (map cadr (car (map cadddr *library*))))))))

(define filler-reward
  (lambda (filler)
    (let
	([priors (lookup-many (car filler) (library-fillers))])
      (if (member? (canonicalize-q-ls (cadr filler)) priors)
	  (thermostat -50 150)
	  (thermostat 100 5)))))
