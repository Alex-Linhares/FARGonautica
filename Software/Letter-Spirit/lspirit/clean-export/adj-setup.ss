...
(define covers
  (lambda (whole)
	...
	([expected (whole-roles whole)]
	 [the-bindings (mapcar get-part-bindings *workspace*)]
	 [bound-roles
	  (uniquify
	   (apply append the-bindings))]
	 [all-bound?
	  (and
	   (eq? (length expected)
		(length (intersect bound-roles expected)))
	   (eq? (length expected) (length *workspace*)))])
      (if all-bound?
	  (let
