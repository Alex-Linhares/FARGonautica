...
(set! *left-edge-list* '(l-edge-lf l-edge-md l-edge-rt))
...
(set! *floor-list* '(floor-bottom floor-middown floor-baseline floor-midline
				  floor-x-height floor-t-height floor-top))
(set! *left-curve-list*
      '(*straight* *slight-left* *square-left* *strong-left*))
(set! *right-curve-list*
      '(*straight* *slight-right* *square-right* *strong-right*))
...
(define compare-nv-type
  (lambda (part nvtype)
	...
	[Roof
	 (list-nv-compare
	  (roof-label (apply max (quanta-verts quanta)))
	  (best-norm *roof-list* norms)
	  *roof-list*)]
	[Floor
	 (list-nv-compare
	  (floor-label (apply min (quanta-verts quanta)))
...
; have to weight these! de-histogram, or average average with best norm
; or, is tip1 at (3) an error for basebar?
(define role-tip-norms
  (lambda (rolename)
    (let*
	([tip-info (lookup-list 'tips (get-norms (eval rolename)))]
	 [coords-1 (map point-coords
		      (map car (cdaar tip-info)))]
	 [coords-2 (map point-coords
		      (map car (cdaadr tip-info)))]
	 [x1-blur (round-3 (average (map car coords-1)))]
	 [y1-blur (round-3 (average (map cadr coords-1)))]
	 [x2-blur (round-3 (average (map car coords-2)))]
	 [y2-blur (round-3 (average (map cadr coords-2)))])
      (list x1-blur y1-blur x2-blur y2-blur))))
