...
(define tip-type
  (lambda (rolename which-tip)
	...
	[down (if tip1?
		  ...
		  (roulette '(down-tip)))]
	[downright (if tip1?
		       (roulette '(up-tip left-tip up-left-tip))
		       (roulette '(down-tip right-tip down-right-tip)))]
	[right (if tip1?
		   (roulette '(left-tip))
		   (roulette '(right-tip)))]
	[upright (if tip1?
		     (roulette '(down-tip left-tip down-left-tip))
		     (roulette '(up-tip right-tip up-right-tip)))]))))
