...
(define get-part-binding-list
    (lambda (part bindings)
      (cond
       ((null? bindings) '())
       ((eq? part (cadar bindings))
	(cons (caar bindings) (get-part-binding-list part (cdr bindings))))
       (else
...
(define part-top-activation
  (lambda (part)
	...
	([bindings (get-part-bindings part)])
	  ...
	  (apply max (mapcar get-activation bindings))
	  0))))
...
(define dampen
  (lambda ()
    (begin
      (mapcar dampen-role-act *roles*)
      (mapcar dampen-whole-act *wholes*)
  ...
  (lambda (part)
	...
	([all-features (get-features part)]
	 ...
	 [spark-it
      ...
      (mapcar spark-it (active-nodes *roles*)))))
    ...
    (case (car item)
      ...
      [contact
       (contact-score (cadr item)
		   (apply append
			  (mapcar cdr (lookup-list 'contact norms))))]
      [shape
...
(define activate-roles-with-part
  (lambda (part gen)
    (let* ([handle-for-part
      ...
      (begin
	(unbind-part part)
	(mapcar handle-for-part spark-roles)
