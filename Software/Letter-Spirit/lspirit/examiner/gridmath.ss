  ...
  (lambda (qls)
	...
	([tips (quanta-real-tips qls)]
	 [start (if (null? tips)
      ...
      (if (< (length tips) 3)
	  (neighbor-sort (list start) (remove start qls))
	  qls))))
...
(define quanta-get-points
 (lambda (qls)
   (uniquify (apply append (mapcar quantum-get-points qls)))))
...
(define quanta-real-tips
 (lambda (qls)
   (find-uniques (apply append (mapcar quantum-get-points qls)))))
...
(define quanta-to-points
 (lambda (qls)
   (if
       (eq? (length qls) 1)
       (quantum-get-points (car qls))
       (let*
       ([primed-list (mapcar quantum-get-points qls)]
...
(define two-points-coords
  (lambda (p-ls)
    (let
	((coords (mapcar point-coords p-ls)))
...
(define part-midpoint
  (lambda (ls)
     (midpoint (mapcar point-coords (head-n-tail (quanta-to-points ls))))))
...
(define curve-sign
  (lambda (end1 mid end2)
	...
	([end1-coord (point-coords end1)]
	 ...
	 [vertical-case (* -1 e2-above-e1 mid-right)]
	 [non-vert-case (* -1 e1-right-e2 mid-above-line)])
      (if
	  ends-vertical?
	  vertical-case
	  non-vert-case))))
  ...
  (lambda (q-ls)
	...
	([p-ls (quanta-to-points q-ls)]
	 ...
	 [point-dist
      ...
      (round-3 (average (mapcar point-dist p-ls)))))))
...
(define mean-point-norm
  (lambda (ls)
    (let
	((sum-x (apply + (mapcar car (mapcar weighted-point ls))))
	 (sum-y (apply + (mapcar cadr (mapcar weighted-point ls))))
	 (weight (apply + (mapcar cadr ls))))
  ...
  (lambda (qls x)
	...
	(let
	    ((x-ward-x
	      (lambda (p)
		(x-wards p x))))
	  (apply max (mapcar x-ward-x (quanta-to-points qls)))))))
...
(define neighbor-watch-match
  (lambda (norm observed)
	...
	([match-item
      ...
      (eval (cons 'and (mapcar match-item norm observed))))))
...
(define mid-quanta-touch?
  (lambda (qls1 qls2)
	...
	((cross-here
      ...
      (eval (cons 'or (mapcar cross-here *mid-quanta-touch-list*))))))
...
(define mid-quanta-touches
  (lambda (qls1 qls2)
	...
	((cross-here
      ...
      (apply + (mapcar cross-here *mid-quanta-touch-list*)))))
