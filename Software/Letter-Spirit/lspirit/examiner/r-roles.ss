...
(define whole-filled?
  (lambda (whole)
    (eval (cons 'and (mapcar filled? (whole-roles whole))))))
...
(define match-two-lists
  (lambda (ls1 ls2)
	...
	([match-all-item
	  (lambda (other)
	    (match-all other ls2))])
      (apply append (mapcar match-all-item ls1)))))
...
(define lists-matchings
  (lambda (ls-ls)
	...
	([len (length ls-ls)])
      (cond
       [(eq? len 0) '()]
       [(eq? len 1) (mapcar list (car ls-ls))]
...
(define spark-combos
  (lambda (expected)
	...
	([in-whole
      ...
      (lists-matchings
       (mapcar in-whole (parts-sparked-roles *workspace*))))))
	...
	([expected (whole-roles whole)]
	 ...
	 [all-bound? (eq? (length expected)
	  ...
	  (let
	    ...
	    (eval (cons 'or (mapcar found-expected sparked))))
	  #f))))
...
(define role-touch-test
  (lambda (whole role)
    (let
      ...
      (apply max (cons 0 (mapcar test-filler
...
(define whole-touch-test
  (lambda (whole)
	...
	([roles (whole-roles whole)]
	 ...
	 [score (apply min (mapcar touch-test-role roles))])
