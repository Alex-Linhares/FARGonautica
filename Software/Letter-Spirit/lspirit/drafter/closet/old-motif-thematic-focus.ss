...
; don't trust this -- it can give bogus matches
(define string-matches
  (lambda (ls1 ls2)
    (intersect (append (sublists ls1) (sublists (reverse ls1)))
	       (list (intersect ls1 ls2)))))

; handy approximation, but flawed
; intersection can give bogus sublists
(define motif-string-matches
  (lambda (sp-type q-ls mtf)
    (let
	([converted-q-ls
	  (case sp-type
	    [*literal-motifs* q-ls]
      ...
      (intersect (motif-sublists sp-type mtf)
		 (list (intersect mtf converted-q-ls))))))
...
(define motif-size-level-search
  (lambda (level-info target-sublists size)
    (let*
	 ...
	 [common
	  (lambda (option)
	    (if (null? target-sublists)
		'()
		(sublists-include target-sublists
				  option)))]
	 [size-limit
	  (lambda (ls)
	    (as-big-as size ls))]
  ...
  (lambda (sp-type sp)
	...
	([prob
	  (case sp-type
	    ...
	    [*val-to-val-nvs* 0.08]
	    [*relative-nvs* 0.25]
	    [*literal-motifs* (power 0.25 (length sp))]
	    [*translate-motifs* (power 0.5 (length sp))]
	    [*turn-180-motifs* (power 0.7 (length sp))]
	    [*turn-90-motifs* (power 0.8 (length sp))]
	    [else (power 0.9 (- (length sp) 1))])])
      (round (* *full-levels* prob)))))
