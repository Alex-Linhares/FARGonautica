...
(define segment-draft-init
  (lambda ()
    (general-draft-init)
    (set! *tip-1* (weight-to-n-roulette (role-tips-1) 6.25))
    (set! *tip-2s* (remove-key *tip-1* (role-tips-2)))
    (set! *tip-2* (weight-to-n-roulette *tip-2s* 6.25))
...
(define loop-dot-draft-init
  (lambda ()
	...
	(begin
	  ...
	  (set! *alt-t2-touch* #f)
	  (set! *tip-1*
		(weight-to-n-roulette (dot-start-points) 6.25))
	...
	(begin
	  ...
	  (set! *alt-t2-touch* #f)
	  (set! *tip-1*
		(weight-to-n-roulette (loop-start-points) 6.25))
...
(define loop-draft-step
  (lambda ()
    (if (or *quit-drawing* (eq? *current-point* *tip-2*)) (printf "~%")
	(begin
	  (take-step)
...
(define bias-points-scores
  (lambda (pt-scores)
	...
	([rule-and-motif-weights
	 ...
	 [quantum-bias
	  (lambda (q)
	    (lookup q rule-and-motif-weights))]
	 [point-options
	  (lambda (pt)
	    (lookup pt *point-list*))]
	 [point-best-option
	 ...
	 [bias-index
      ...
      (score-lists-multiply pt-scores bias-index))))
...
(define tip-candidates
  (lambda (tip-info end-info)
	...
	([end-tips (score-list-max-collapse
	 ...
	 [best (apply max (map cadr raw-scored))])
