...
(define examine-gridletter
  (lambda (attempt qls)
    (begin
	  ...
	  (examiner-cleanup))
      (printf "~s ~s ~s ~s ~s " *the-font* attempt *answer*
	      (get-category *answer*) *codelets-run*)
      (cond
  ...
  (lambda (lt hex runs)
	...
	(let* ([code (string->list hex)]
	  ...
	  (begin
		...
		(examiner-cleanup))
	    (printf "~a ~a as ~a ac ~a cd ~a tm ~a an ~a ~a ~a~%"
		    *the-font* lt ans-let act step tmp grade hex (parts-score))
	    (collect-stats lt hex (- runs 1)))))))
...
; lacks, from set of 23:
; checkmark, funtnip, intersect, sluice, three-d, weird-arrow
