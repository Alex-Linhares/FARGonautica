...
type
    ...
    TPiece = class
                 ...
                 Procedure map_Current_Terrain; virtual;
    ...
    TPawn = class (Tpiece)
                  ...
                  Procedure map_Current_Terrain; override;
            end;
    TKing = class (Tpiece)
                  Procedure Load_Data; override;
                  function getnextmoves(P:Tsquare):Tlist; override;
...
begin
     ...
     begin
          ...
          begin
               ...
               //2010 change; from (aux<0) or (aux=110000) to...
               if ( (aux<-1) or (aux>=10000)) then
                  result:=false;
          end;
     end;
...
Procedure TPawn.map_Current_Terrain;
var x,y: integer;

begin
    {2010 change}
    //TPiece.map_Current_Terrain;

    if white then
    begin
        if (Mobility[sq.x,8].distance=10000) then
            Mobility[sq.x,8].distance:=Mobility[sq.x,7].distance-1;
    end
    else
    begin
        if (Mobility[sq.x,1].distance=10000) then
            Mobility[sq.x,1].distance:=Mobility[sq.x,2].distance-1;
    end;
...
begin
     ...
     for x := 1 to 8 do
         ...
         begin
              ...
              aux:=Mobility[x,y].distance;
              if aux<0 then aux:=-aux;
              if (aux>=distance)then
                 List.Add(psquare);
              //else if ( (PieceType=Pawn) and ((y=1)or (y=8)) ) then
              //   List.Add(psquare);
         end;
     result:=list;
end;
