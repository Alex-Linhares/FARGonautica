...
type
     ...
     TImpulseAbstractRole = class (TImpulse)
                                  ...
                                  Function GetDestinationValueForIntensity:Integer;   virtual;
                            end;
     ...
     TImpulsePromotionRole = class (TImpulseAbstractRole)
                                  Constructor create (P:TParamAbstractRole; U: TUrgency);
                                  procedure Fire; Override;
                                  Function GetDestinationValueForIntensity:Integer; override;
                                  end;
...
{
2010--> REFACTOR INTENSITY

Problem:
When the Black Pawn at H7 might attack the White King at E1
at the fastest possible mate, with an intensity of around 560000, intensity is the problem.

There are these issues:

II)   the major issue is that a relation's intensity is dependent on the distance, but there
is low discrimination between large and small distances;

III) There should be a comparison between distances involved in interceptions; with
the highest getting lower intensity;

IV) Why is dynamicValue at 101-XXX?? Because of constant KingValue

try to solve one at a time please dude;
}

{Function TImpulseAbstractRole.GetIntensity:integer;
var proximity, value1 :integer;
begin
     ...
     Proximity:= 63 - Parameters.Trajectory.count;
     if (proximity<=0) then Proximity:=1;

     value1:=(101-parameters.origin.DynamicValue);  //Note that KingValue was 100;

     Result:= (value1*GetDestinationValueForIntensity*Proximity);
end;}


Function TImpulseAbstractRole.GetIntensity:integer;
var proximity, DeltaBetweenOriginAndDestination :integer;
begin
     {First measure of intensity, among other things
     intensity is a function of the value of the pieces and of the distances involved}
     ...
     DeltaBetweenOriginAndDestination:=  GetDestinationValueForIntensity - parameters.origin.DynamicValue;
     //what if this value is negative?  We have to add some base rate here...
     DeltaBetweenOriginAndDestination:= DeltaBetweenOriginAndDestination +  KingValue;

     Result:= (DeltaBetweenOriginAndDestination*Proximity);
end;

Function TImpulseAbstractRole.GetDestinationValueForIntensity:Integer;
begin
  result:=parameters.destination.DynamicValue;
end;


Function TImpulsePromotionRole.GetDestinationValueForIntensity:Integer;
begin
  result:=Queenvalue;
end;
...
begin
     ...
     begin
          ...
          Node.Activate(Intensity*Intensity);
     end;
end;
