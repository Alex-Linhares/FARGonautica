...
for i = 1:2:nargin-1                  % get optional args
    switch varargin{i}
        ...
        case 'node_labels', labels  = varargin{i+1}; %node_labels[i+1];  %node_labels[i+1];  %LINHARES SPAM SPAM SPAM%varargin{i+1};
        case 'fontsize',  fontsize = varargin{i+1}; 
        case 'node_shapes', node_t  = varargin{i+1};  node_t = node_t(:);
        case 'nodemult',  nodemult = varargin{i+1};
...
if ~isempty(idx1),
    [h1 wd1] = textoval(x(idx1), y(idx1), labels(idx1), fontsize, color, nodemult);
end;
...
function [t, wd] = textoval(x, y, str, fontsize, c, nodemult)
...
for i = 1:N,
    ...
    sz = get(tx, 'Extent');
    wy = sz(4);
    wx = max(1/3*sz(3), wy); %2/3*sz(  -->LINHARES
    wx = nodemult * wx ;        %  0.9 might want to play with this .9 and .5 coefficients 
    wy = nodemult * wy;
    ptc = ellipse(x(i), y(i), wx, wy, c(i,:)); %Do we need the ovals
%    anyway, Linhares??
...
deflen        = 12;  %  16           
deflen        = 5;  %  3 for whitman class report, 5 for cog sci 08 

defbaseangle  = 45;  %  90
defbaseangle  = 90;  %  90

deftipangle   = 15;
deftipangle   = 12;  % cog sci 2008
