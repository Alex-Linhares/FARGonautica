...
type
  ...
  private
    Function GetMeaningOfConnotation(C: TConnotation; var Meaning: TList):TList;
...
begin
    AvailableSTMItems:=TList.Create;
    NeededItems:=TList.Create;
    for I := 0 to STM.Count - 1 do
    begin
       C:=STM[i];
       if C.InheritsFrom(TBrick) then
          AvailableSTMItems.add(C);
    end;
    NumAvailable:=0;
    NumNeeded:=0;
    NeededItems:=ReceivedConnotation.ListAllConnotations(NeededItems);

    i:=0;
    while (i<= NeededItems.Count-1) do
    begin
        C:=NeededItems[i];
        If (C.InheritsFrom(TNumberInteger) and not C.inheritsFrom(TResult) ) then
        begin
    ...
    if numneeded=0 then res:=0 else res:=NumAvailable/NumNeeded;
    ...
    begin
        ...
        begin
            ...
            begin
                ...
                if Target.Value<Number.Value then
                    NumboSlipnet.LinkNodes(TargetN, NumboSlipnet.GetNode(Op), 1)
                else NumboSlipnet.LinkNodes(NumboSlipnet.GetNode(Op), TargetN, 1);
                Desirable.Add(Op);
            end;
        end;
        i:=i+1;
...
Function RemoveThoseLessRelevantThan(L:TList; R:real):TList;
...
var rounds, x:integer;
    ...
    Desirable, Desirable2, Desirable3:TList;
    S:String;
    I, J, k: Integer;
    cutpoint:real;
...
begin
    formcreate(self);
    //desirable:=tlist.create;
    memo1.Lines.Add('=============================================================');
    C:=target;
    memo1.Lines.Add('FOR TARGET '+PrintConnotationString(C));

    Desirable:=GetMeaningOfConnotation (C, desirable);
    ...
    begin
        ...
        Desirable2:=GetMeaningOfConnotation(C, Desirable2);
        ...
        begin
            ...
            Desirable3:=GetMeaningOfConnotation(C, Desirable3);
...
begin
    ...
    SubCog:=GetMeaningOfConnotation (CurrentIdea, SubCog);
    SubCog:= RemoveThoseLessRelevantThan(SubCog, -0.1);
    //PrintConnotations(SubCog);
    ...
    begin
        ...
        if delta <= 0 {tolerance[level]} then
        begin
            //accept;
            CurrentIdea:=TConnotation(SubCog[0]);
            effort:=0;
        ...
        Tolerance[level]:=CurrentIdea.Relevance(Target, STMAvailability(CurrentIdea));
        CurrentIdea:=TConnotation(SubCog[0]);
        PrintConnotationString(CurrentIdea);
...
Function TForm1.GetMeaningOfConnotation(C: TConnotation; var Meaning: TList):TList;
begin
